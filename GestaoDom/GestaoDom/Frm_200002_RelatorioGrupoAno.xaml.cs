/*
Frm_200002_RelatorioGrupoAno.xaml.cs
...
{
    /// <summary>
    /// Interaction logic for Frm_200002_RelatorioGrupoAno.xaml
    /// </summary>
    public partial class Frm_200002_RelatorioGrupoAno : Window
    {
        ...
        public Frm_200002_RelatorioGrupoAno(int loginUserSequence, string loginUserId, string loginUserType)
        {
            InitializeComponent();
        ...
        private void QueryValue()
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                try
                {
                    ...
                    string create = "CREATE TABLE tmp " +
                        "SELECT LEFT(md_codartigo, 8) AS cod, fd_datadoc, md_quantidade, md_precofinal " +
            ...
            string filePath = @"c:\Despesas\Relatórios\GrpAnoMesValor_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em valor por grupo/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");

                var columns = new[] {
                new { Width = 50, Header = "Grupo" },
                ...
                using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
                {
                    try
                    {
                        ...
                        string query = "SELECT grp_descr, " +
                            ...
                            "FROM tmp " +
                            "LEFT JOIN tbl_0203_grupos ON tmp.cod = tbl_0203_grupos.grp_codigo " +
                            "GROUP BY grp_descr " +
                            "ORDER BY grp_descr";
                        using (MySqlCommand cmd = new MySqlCommand(query, conn))
                        {
                            using (MySqlDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    string md_grpdescr = reader.GetString("grp_descr");
                                    ...
                                    outStream.Write(md_grpdescr.PadRight(columns[0].Width) + " | ");
        ...
        private void QueryQuantity()
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                try
                {
                    conn.Open();
                    string create = "CREATE TABLE tmp " +
                         "SELECT LEFT(md_codartigo, 8) AS cod, fd_datadoc, md_quantidade " +
                         "FROM tbl_0302_movimentosdebito_det " +
                         "LEFT JOIN tbl_0301_movimentosdebito ON tbl_0302_movimentosdebito_det.md_id_fatura = tbl_0301_movimentosdebito.fd_id_fatura " +
                         "WHERE LEFT(fd_datadoc, 4) = ? ";
                    using (MySqlCommand cmd = new MySqlCommand(create, conn))
            ...
            DateTime now = DateTime.Now;
            string formattedDate = now.ToString("yyyyMMddHHmmss") + ".txt";
            string filePath = @"c:\Despesas\Relatórios\GrpAnoMesQtd_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em quantidade por grupo/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");
