/*
Frm_200004_RelatorioFamAno.xaml.cs
...
{
    /// <summary>
    /// Interaction logic for Frm_200004_RelatorioFamAno.xaml
    /// </summary>
    public partial class Frm_200004_RelatorioFamAno : Window
    {
        ...
        public Frm_200004_RelatorioFamAno(int loginUserSequence, string loginUserId, string loginUserType)
        {
            InitializeComponent();
        ...
        private void QueryValue()
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                try
                {
                    ...
                    string create = "CREATE TABLE tmp " +
                        "SELECT LEFT(md_codartigo, 2) AS cod, fd_datadoc, md_quantidade, md_precofinal " +
            ...
            string filePath = @"c:\Despesas\Relatórios\FamAnoMesValor_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em valor por familia/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");

                var columns = new[] {
                new { Width = 50, Header = "Familia" },
                ...
                using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
                {
                    try
                    {
                        ...
                        string query = "SELECT fam_descr, " +
                            ...
                            "FROM tmp " +
                            "LEFT JOIN tbl_0201_familias ON tmp.cod = tbl_0201_familias.fam_codigo " +
                            "GROUP BY fam_descr " +
                            "ORDER BY fam_descr";
                        using (MySqlCommand cmd = new MySqlCommand(query, conn))
                        {
                            using (MySqlDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    string md_famdescr = reader.GetString("fam_descr");
                                    ...
                                    outStream.Write(md_famdescr.PadRight(columns[0].Width) + " | ");
        ...
        private void QueryQuantity()
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                try
                {
                    conn.Open();
                    string create = "CREATE TABLE tmp " +
                         "SELECT LEFT(md_codartigo, 2) AS cod, fd_datadoc, md_quantidade " +
                         "FROM tbl_0302_movimentosdebito_det " +
                         "LEFT JOIN tbl_0301_movimentosdebito ON tbl_0302_movimentosdebito_det.md_id_fatura = tbl_0301_movimentosdebito.fd_id_fatura " +
                         "WHERE LEFT(fd_datadoc, 4) = ? ";
                    using (MySqlCommand cmd = new MySqlCommand(create, conn))
            ...
            DateTime now = DateTime.Now;
            string formattedDate = now.ToString("yyyyMMddHHmmss") + ".txt";
            string filePath = @"c:\Despesas\Relatórios\FamAnoMesQtd_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em quantidade por familia/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");
