/*
FRM_200001_RelatorioArtAno.xaml.cs
...
{
    /// <summary>
    /// Interaction logic for Frm_200001_RelatorioArtAno.xaml
    /// </summary>
    public partial class Frm_200001_RelatorioArtAno : Window
    {
        ...
        public Frm_200001_RelatorioArtAno(int loginUserSequence, string loginUserId, string loginUserType)
        {
            InitializeComponent();
        ...
        private void QueryValue()
        {
            ...
            string filePath = @"c:\Despesas\Relatórios\ArtAnoMesValor_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em valor por artigo/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");

                var columns = new[] {
                new { Width = 50, Header = "Artigo" },
                    ...
                    try
                    {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, conn))
                        {
                            ...
                            using (MySqlDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    string md_artdescr = reader.GetString("art_descr");
                                    ...
                                    outStream.Write(md_artdescr.PadRight(columns[0].Width) + " | ");
        ...
        private void QueryQuantity()
        {
            ...
            DateTime now = DateTime.Now;
            string formattedDate = now.ToString("yyyyMMddHHmmss") + ".txt";
            string filePath = @"c:\Despesas\Relatórios\ArtAnoMesQtd_" + ano + "_" + formattedDate;
            using (StreamWriter outStream = new StreamWriter(filePath))
            {
                outStream.WriteLine($"{"Relatório de gastos em quantidade por artigo/ano/mês",-100}{" " + dataPedido.ToString("dd/MM/yyyy"),146}");
                outStream.WriteLine($"{" ",-100}{" " + dataPedido.ToString("HH:mm:ss"),146}");
                outStream.WriteLine($"{"Ano: " + ano,-100}{" " + loginUserId,146}");
