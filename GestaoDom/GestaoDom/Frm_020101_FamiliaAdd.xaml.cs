...
{
    /// <summary>
    /// Interaction logic for Frm_020101_FamiliaAdd.xaml
    /// </summary>
    public partial class Frm_020101_FamiliaAdd : Window
    {
        private readonly string loginUserId;
        public Frm_020101_FamiliaAdd(string loginUserId)
        {
            InitializeComponent();
        ...
        private void LoadNumeracao()
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                ...
                try
                {
                    ...
                    string query = "SELECT MAX(fam_codigo) FROM tbl_0201_familias";
                ...
                catch (Exception)
                {
                    ...
                    System.Windows.MessageBox.Show("Erro ao carregar númeração (Erro: N1003)!");
                }
        ...
        public bool ValidarFamilia(string familia)
        {
            ...
            using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
            {
                try
                {
                    ...
                    string query = "SELECT COUNT(*) FROM tbl_0201_familias WHERE fam_descr = ?";
        ...
        private void Btn_Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (txt_Descr.Text != "")
            {
                ...
                if (txt_Descr.Text.Trim() == txt_Descr.Text)
                {
                    ...
                    if (!ValidarFamilia(txt_Descr.Text))
                    {
                        // Obtem ligação
                        using (MySqlConnection conn = Cls_0001_DBConnection.GetConnection())
                        {
                            ...
                            try
                            {
                                conn.Open();
                                // Definição do insert 
                                string query = "INSERT INTO tbl_0201_familias(fam_codigo, fam_descr, fam_status, fam_usercreate, fam_datecreate, fam_timecreate, fam_userlastchg, fam_datelastchg, fam_timelastchg) " +
                                    "                       VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
                                ...
                                // Fecha o formulário                            
                                this.Close();
                                System.Windows.MessageBox.Show("Familia inserida com exito!");
                            }
                            catch (Exception)
                            {
                                // mensagem de erro da ligação
                                System.Windows.MessageBox.Show("Ocorreu um erro ao ligar à base de dados (Erro: I1005)!");
                                return;
                            }
                        }
                    }
                    ...
                    {
                        System.Windows.MessageBox.Show("Já existe uma familia com este nome, escolha uma nova descrição!");
                    }
                }
                else
                {
